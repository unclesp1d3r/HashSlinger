...
public class UpdateFileRecordHandler : IRequestHandler<UpdateFileRecordCommand>
{
    ...
    {
        Shared.Models.File fileRecord = _dbContext.Files.FirstOrDefault(x => x.Id == request.FileId)
                                        ?? throw new InvalidOperationException("FileRecord is null");

        var fileName = fileRecord.FileName;
        FileInfo? file = await _storageService.GetFileInfoAsync(fileName, "files").ConfigureAwait(true);
